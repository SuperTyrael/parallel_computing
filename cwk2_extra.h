...
int getNFromCommandLine(int argc, char **argv, int rank, int numprocs)
{
    ...
    {
        if (rank == 0)
            printf("Error: Must enter the matrix and vector size N on the command line.\n");
        MPI_Finalize();
        return -1;
    }
    ...
    {
        if (rank == 0)
            printf("Could not convert the command line argument '%s' to a positive integer.\n", argv[1]);
        MPI_Finalize();
        return -1;
    }
    if (N % numprocs)
    {
        if (rank == 0)
            printf("The problem size N=%i is not a multiple of the number of processes=%i.\n", N, numprocs);
        MPI_Finalize();
        return -1;
    }
